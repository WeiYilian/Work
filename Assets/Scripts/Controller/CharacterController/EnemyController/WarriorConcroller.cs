...
public class WarriorConcroller : MonoBehaviour
{
    ...
    public void KickOff()
    {
        if(attackTarget != null && transform.IsFacingTarget(attackTarget.transform)/*扩展方法*/)
        {
            ...
            direction.Normalize();//将向量量化，取方向（有正负）
            
            attackTarget.GetComponent<CharacterController>().Move(direction * kickForce);//给一个力
            attackTarget.GetComponentInChildren<Animator>().SetTrigger("Repelled");
        }
    }
