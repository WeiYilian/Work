...
public static class DataManager
{
    ...
    public static void UpdataUserMessage(List<string> users)
    {
        ...
        {
            conn.Open();
            string sqlQuary = $"update usermessage set username = '{users[1]}',password = '{users[2]}',age = '{users[3]}',gender = '{users[4]}',level = '{users[5]}',exp = '{users[6]}',health = '{users[7]}'  where id = '{users[0]}'";
            Debug.Log("修改成功");
            MySqlCommand comd = new MySqlCommand(sqlQuary, conn);
            comd.ExecuteReader();
        }
