...
public class AudioManager : MonoBehaviour
{
    ...
    private void Init()
    {
        audioList = new List<AudioClip>()
        {
            ...
            GameFacade.Instance.LoadAudioClip("OpenBox"),//9，开启宝箱声音
            GameFacade.Instance.LoadAudioClip("SwingEmpty"),//10，武器挥空声音
            GameFacade.Instance.LoadAudioClip("Summon"),//11，召唤骷髅士兵声音
            GameFacade.Instance.LoadAudioClip("Walk"),//12，走路声音
            GameFacade.Instance.LoadAudioClip("Run"),//11，跑步声音
            GameFacade.Instance.LoadAudioClip("失败音效"),//12，失败声音
            GameFacade.Instance.LoadAudioClip("胜利音效")//13，胜利声音
        };
    }
    ...
    public void PlayMoveAudio(int index,string audioNme)
    {
        if (!transform.GetChild(index).GetComponent<AudioSource>().isPlaying)
            PlayAudio(index,audioNme,3,true);
    }
