    ...
    public override void OnEnter()
    {
        ...
        UITool.GetOrAddComponentInChildren<Button>("BtnPlay").onClick.AddListener(() =>
        {
            ...
            if (isLogin)
            {
                ...
                GameLoop.Instance.controller.SetState(new LoadScene(GameLoop.Instance.controller.mSceneState.mController));
            }
            else
            {
                GameLoop.Instance.Push((new NoticePanel("用户名或密码错误！！")));
            }
        });
    }
}
